...
Git が入っているターミナルを開き、次の2つのコマンドを実行しましょう。
...
<Callout type="warning">
    ダミーでないメールアドレスを登録する場合、ソースコードを GitHub で公開するとメールアドレスが公開されてしまうので注意してください。
...
<Callout type="error">
    `.pub` をつけ忘れると秘密鍵がコピーされることになり、公開すると人生が終わるので注意してください。
</Callout>

コピーしたものをGitHubの [SSH 鍵の設定](https://github.com/settings/keys) に登録します。

`New SSH key` を押し、`Title` は適当な名前を入力、`Key` にコピーしたものを貼り付けしてください。

`Add SSH key` をクリックして完了したら、SSH接続のテストをしましょう。
```sh
